...
export const tilEntries: TilEntry[] = [
  {
    ...
    content: `AWS ALB now supports URL rewrites using the **transform action** - no need to rely on redirects or backend rewrites anymore! This is particularly useful for path-based routing scenarios where you want to modify the request before it reaches your backend.
...
- [AWS Load Balancer Controller annotations](https://kubernetes-sigs.github.io/aws-load-balancer-controller/v2.14/guide/ingress/annotations/#transforms)`,
  },
  {
    ...
    content: `As an SRE, we're often measuring things in units of time. Eg. billions of requests per month, cost of running x in aws for y time, etc.
...
\`4,294,967,296 seconds / (365.25 * 24 * 3600) ≈ 136 years from 1970 = 2106\``,
  },
  {
    ...
    content: `This deployment will eventually get OOMKilled as it tries to allocate more memory than the limit set.
...
\`\`\``,
  },
  {
    ...
    content: `I was creating some basic app deployments in ArgoCD and was using a multi source app to pass in values to a Helm chart. Little did I know, that any Kubernetes resource in the path of the ref source will actually also be included in the rendered template.
...
Being able to essentially put any supporting resource in the \`baz/bar/foo\` directory makes things easier if the chart does not support the common pattern of extraObjects / extraManifests.`,
  },
  {
    ...
    content: `I've been using \`fzf\` for a while now, and it has completely transformed my command line experience. The ability to fuzzy-find files, commands, and even git branches is a game changer. If you haven't tried it yet, I highly recommend giving it a go!

![fzf.gif](/images/til/fzf.gif)
...
\`\`\``,
  },
  {
    ...
    content: `[ncspot](https://github.com/hrkfdn/ncspot) is a cross-platform Spotify client for the terminal written in Rust. Lightweight alternative to the bloated Spotify desktop app.

![ncspot](/images/til/ncspot.png)`,
  },
  {
    ...
    content: `Use Renovate packageRules to migrate action references across repos when Github org needs to be changed:
...
This was super helpful when we migrated our shared workflows over to a new org and needed to update 100s of repos. We then we able to handover the work for merging to the devs responsible for each repo.`,
  },
  {
    ...
    date: "2025-09-10",
    tags: ["terraform", "hcl"],
    content: `Its the simple stuff I always forget. Remove resource from state but don't destroy it.
